...
public class onPlayerInteractEntity extends SimpleListener implements Listener, EventExecutor {
    ...
    public void execute(@NotNull Listener listener, @NotNull Event event) throws EventException {
        ...
        if(e.getRightClicked() instanceof Villager) {
            e.setCancelled(true);
            if(e.getRightClicked().getCustomName().equals("§e§lМАГАЗИН")) e.getPlayer().openInventory(this.getPlugin().getPlayer(e.getPlayer()).getShopInventory());
            if(e.getRightClicked().getCustomName().equals("§e§lУЛУЧШЕНИЯ")) {

                if(this.getPlugin().getPlayer(e.getPlayer()).hasTeam()) e.getPlayer().openInventory(this.getPlugin().getPlayer(e.getPlayer()).getTeam().getUpgradesInventory());
