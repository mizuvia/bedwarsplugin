...
public class Potions implements IGUI{
    private final Game game;

    public Potions(Game game) {
        this.game = game;
    }
    ...
    public void onGUIClick(Player whoClicked, int slot, ItemStack clickedItem) {
        switch (slot) {
            ...
            case 30, 31, 32 -> this.getInventory().makeTrade(this.getGame().getPlugin().getPlayers().get(whoClicked.getUniqueId()), clickedItem);
        }
    }

    @NotNull
    @Override
    public PotionsInventory getInventory() {
        return this.getGame().getPotionsInventory();
    }
}
